...
class Manager:
    ...
    def __init__(
        ...
        self._accumulation_grad = accumulation_grad
    ...
    def allreduce(
        ...
        * The tensor must be zeroed before being used as it may be corrupted.
        ...
        if not self.is_participating():
            tensor.zero_()
        elif self._accumulation_grad:
            tensor /= self._accumulation_steps
    ...
    def _async_quorum(
        ...
        if quorum_id != self._quorum_id:
            ...
            self._logger.info(
                f"reconfiguring for {quorum_id=} {store_prefixed_addr=}, "
                f"{self._participating_replica_world_size=}, "
                f"{self._participating_replica_rank=}"
            )
    ...
    def state_dict(self) -> Dict[str, int]:
        ...
        """
        print("Getting manager state dict, ")
        return {
            "step": self._step,
    ...
    def reconfigure_dataloader(self):
        ...
        if hasattr(dataloader, "batch_sampler"):
            dataloader.batch_sampler.sampler.set_epoch(self._epoch)
        else:
