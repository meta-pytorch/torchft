...
def build_gpu_pod_template(image: str, gpus_per_host: int) -> V1PodTemplateSpec:
...
class MonarchKubernetes:
    ...
    async def get_or_create_job(self, mesh_name: str) -> None:
        ...
        if self.image is not None:
            pod_template = build_gpu_pod_template(self.image, self.gpus_per_host)
            job.add_mesh(mesh_name, num_replicas=1, pod_template=pod_template)
        else:
